...
for (( i=0; i<${#kinlist}; i++ )); do
  ...
  kin=$(printf "%02d" $kin)
  csvFilePrefix="results/db/${dateConf}_kin${kin}_"
  ...
  if [ $findSetID == 1 ]; then
    ...
    if [ ${#thisKinFiles[@]} -eq 0 ]; then
      setID='000'
      echo "Found no previous set for ${dateConf} Kin${kin}. SetID: ${setID} "
    else
  ...
  echo "Backing up all result files for ${dateConf} Kin${kin} Set ${setID}"
  mkdir -p results/saved
  tar jcf results/saved/results_${dateConf}_kin${kin}_${setID}.tar.bz2 results/${dateCmd}/kin${kin} ${csvFile}
done
