...
void draw(int arm, int hadron, PlotType_t type = PlotAngles)
{
  ...
  if(arm == kHArm) {
    ...
    cuts = cuts && "Harm.HCAL_box.npart_CAL>0&&Harm.HCAL_box.mid==0" &&
      Form("Harm.HCAL_box.edep>%0.2f",kMinEDep);
  } else if ( arm == kEArm ) {
    histName = Form("hEArm%s%d",histPostfix.Data(),hadron);
    cuts = cuts && "Earm.BBCal.npart_CAL>0&&Earm.BBCal.mid==0" &&
      Form("Earm.BBCal.edep>%0.2f",kMinEDep);
  } else if ( arm == kBothArms ) {
    histName = Form("hBothArms%s%d",histPostfix.Data(),hadron);
    cuts = cuts && "Harm.HCAL_box.npart_CAL>0&&Earm.BBCal.npart_CAL>0" &&
      "Harm.HCAL_box.mid==0&&Earm.BBCal.mid==0" &&
      Form("Harm.HCAL_box.edep>%0.2f&&Earm.BBCal.edep>%0.2f",
          kMinEDep,kMinEDep);
  }
...
void  plotAcceptance(int kin = 1)
{
  ...
  std::cout << "Found "
    ...
    << " rootfiles with total entries: " << chain->GetEntries() std::endl;

  chain->SetAlias("theta","ev.th*57.2958");
  chain->SetAlias("phi","ev.ph*57.2958");
  chain->SetAlias("bbx","(Earm.BBCal.x*TMath::Cos(gen.thbb)-"
      "Earm.BBCal.z*TMath::Sin(gen.thbb))");
  chain->SetAlias("bby","(Earm.BBCal.y)");
  chain->SetAlias("bbz","(Earm.BBCal.x*TMath::Sin(gen.thbb)+"
      "Earm.BBCal.z*TMath::Cos(gen.thbb))");
  chain->SetAlias("hcalx","(Harm.HCAL_box.x*TMath::Cos(-gen.thsbs)-"
      "Harm.HCAL_box.z*TMath::Sin(-gen.thsbs))");
  chain->SetAlias("hcaly","(Harm.HCAL_box.y)");
  chain->SetAlias("hcalz","(Harm.HCAL_box.x*TMath::Sin(-gen.thsbs)+"
      "Harm.HCAL_box.z*TMath::Cos(-gen.thsbs))");
