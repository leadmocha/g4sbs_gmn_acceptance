...
const double kExclusionZoneCutStep = 0.05;  // in m
const int kExclusionZoneCutMaxSteps = 5;
...
void GMnAcceptance::Init()
{
  ...
  fhAnglesSigmaN = MakeAnglesProfile("Neutron d#sigma/d#Omega*sin#theta_{e} (#mub)","fhAnglesSigmaN");
  fhAnglesSigmaP = MakeAnglesProfile("Proton d#sigma/d#Omega*sin#theta_{e} (#mub)","fhAnglesSigmaP");
    ...
    int hadron)
{
  TH2F *histo = new TH2F(TString::Format("%s%d_v%g",name,hadron,fVOff),
      TString::Format("HCal Position%s%s: %s;x (m);y (m)",(title==""?"":" "),
...
TH2F* GMnAcceptance::MakeBBPositionHisto(const char *title, const char* name)
{
  TH2F *histo = new TH2F(TString::Format("%s_v%g",name,fVOff),
      TString::Format("BBCal Position%s%s;x (m);y (m)",(title==""?"":" "),
        title),
...
GMnResultsSigmaSol_t GMnAcceptance::ComputeAcceptance(TH2F *nEff, TH2F *pEff)
{
  ...
  // Convert results for solid angle from degres back to radians
  result.solidN *= TMath::Power(TMath::Pi()/180.0,2);
  result.solidP *= TMath::Power(TMath::Pi()/180.0,2);
  ...
  for(int bx = 2; bx <= nbx; bx++) {
    for(int by = 2; by <= nby; by++) {
      ...
      if( map->GetBinContent(bx,by) && sum == 0 ) {
        ...
        // Somehow this bin is empty, but it's neighbors are not. Better
        // include it.
      }
    }
  }
...
void GMnAcceptance::DetermineAcceptanceMap()
{
  ...
  for(int bx = 1; bx <= nbx; bx++) {
    for(int by = 1; by <= nby; by++) {
      ...
      //en = fhAnglesENRG[3]->GetBinError(b);
      //ep = fhAnglesEPRG[3]->GetBinError(b);
      diff = cn-cp;
      avg = (cn+cp)/2.;
      pdiff = diff/avg;
      if(cn > 0 && cp > 0 && TMath::Abs(pdiff) <= kEfficiencyDiffTolerance
        ...
        fhAnglesSigmaN->SetBinEntries(b,0);
        fhAnglesSigmaP->SetBinEntries(b,0);
        //fhAnglesSolidAngleN->SetBinContent(b,0);
        //fhAnglesSolidAngleP->SetBinContent(b,0);
        //fhAnglesSolidAngleN->SetBinEntries(b,0);
        //fhAnglesSolidAngleP->SetBinEntries(b,0);
      }
    }
  }
